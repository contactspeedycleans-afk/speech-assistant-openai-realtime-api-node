...
console.log("Appointment date/time set.");
    ...
    const notesInspector = await page.evaluate(() => {
      const visible = el => {
      ...
      const controls = Array.from(
        document.querySelectorAll(
          'textarea, input, select, button, [role="button"]'
        )
      )
        .filter(visible)
        .map((el, index) => {
    ...
    console.log("");
    console.log("===== NOTES ALL CONTROLS =====");
    console.log(JSON.stringify(notesInspector, null, 2));
    console.log("===== END NOTES ALL CONTROLS =====");
    console.log("");
