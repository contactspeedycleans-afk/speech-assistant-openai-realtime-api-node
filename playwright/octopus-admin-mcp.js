...
const server=http.createServer(async(req,res)=>{
 ...
 if(u.pathname==="/register"&&req.method==="POST"){const data=JSON.parse(await body(req)||"{}");return json(res,201,{client_id:"cgpt_"+randomBytes(18).toString("base64url"),client_id_issued_at:Math.floor(Date.now()/1000),redirect_uris:data.redirect_uris||[],token_endpoint_auth_method:"none",grant_types:["authorization_code","refresh_token"],response_types:["code"]})}
