export function buildSessionContext({
    ...
    const customerName = databaseCustomerName || outboundCustomerName || '';

    const customerAddress = [
        customer?.address || outboundLead.address || [outboundLead.streetNumber, outboundLead.street].filter(Boolean).join(' '),
        customer?.city || customer?.suburb || outboundLead.city,
        customer?.state || outboundLead.state,
        customer?.zip || customer?.postcode || outboundLead.zip
    ]
        .filter(Boolean)
        .join(', ')
        .trim();
...
Pricing gate: never quote a price until one-time versus recurring is known. If frequency is unknown, ask that one question and stop. Quote $150 only after the customer selects one-time cleaning. For recurring cleaning, quote only the chosen frequency: weekly $112.50, biweekly $120, or monthly $127.50. Then recommend Forever Clean once as the best value: $250 annually and $82.50 per cleaning. Do not use $150 as a generic quote or as the opening price for a recurring lead.

Do not act like an order taker. Once the cleaning type is known, give a short benefit-focused description of what that specific service includes before pricing and scheduling. Sell the visible result, included professional supplies, convenience, and Forever Clean savings. Then use any date and time already stated to close confidently instead of asking for them again.
`;
...
Determine the customer type from RETURNING CUSTOMER FOUND, BOOKING CUSTOMER MATCHED BY CALLER PHONE, CUSTOMER BOOKING HISTORY, and the outbound instructions.

If an existing customer or existing booking is present, this is an account-service call unless the instructions explicitly say the customer requested a separate new quote or new job. Use their saved appointment, service, membership, and recurring information immediately. Do not ask one-time versus recurring, do not quote $150, and do not restart the new-customer intake flow. Speak about their existing service as something already on the account.

For confirmations, follow-ups, billing, reschedules, or cancellations, stay focused on that purpose. Ask only for a detail that is both missing and required to finish the task.

Only use the new-lead sales sequence when there is no existing booking/customer evidence or the instructions clearly identify a new service request.
