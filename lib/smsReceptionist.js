    ...
    },
    {
        ...
        function: {
            ...
            parameters: {
                ...
                required: ['customerName', 'customerPhone', 'serviceAddress', 'serviceType', 'requestedDate', 'arrivalWindow', 'durationMinutes', 'recurringFrequency', 'quotedPrice', 'customerConfirmed'],
                additionalProperties: false
            }
        }
    },
...
async function createOctopusBooking(args, customerPhone) {
    ...
    const required = ['customerName', 'serviceAddress', 'serviceType', 'requestedDate', 'arrivalWindow'];
...
export async function runSmsReceptionist({
    ...
    const identity = technician
        ...
        : customer
            ? { role: 'customer', id: customer.id, name: `${customer.first_name || ''} ${customer.last_name || ''}`.trim(), email: customer.email || '' }
            : { role: 'unknown', name: '' };
