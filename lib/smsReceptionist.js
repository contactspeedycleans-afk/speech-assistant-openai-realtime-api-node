        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    durationMinutes: { type: 'integer', minimum: 60 },
...
async function createOctopusBooking(args, customerPhone) {
    ...
    const specialEquipmentBooking = /carpet|power\s*wash/i.test(
        `${args.serviceType || ''} ${args.specialRequests || ''}`
    );
    const minimumDurationMinutes = specialEquipmentBooking ? 60 : 120;
    if (missing.length || Number(args.durationMinutes) < minimumDurationMinutes || !Number.isFinite(Number(args.quotedPrice))) {
...
export async function runSmsReceptionist({
    ...
    for (let round = 0; round < 5; round += 1) {
        ...
        if (!message) throw new Error('Lisa generated no SMS response.');
    ...
    throw new Error('Lisa used too many SMS tool steps without completing a response.');
}
