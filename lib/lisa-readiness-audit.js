...
export async function sendOwnerReadyNotice(client, profileResult) {
  ...
  const prior = await client.messages.list({to,limit:50});
  console.log('LISA_NOTES_ALERT_DELIVERY=' + JSON.stringify(prior.filter(m=>m.body?.includes('Lisa notes review ')).map(m=>({sid:m.sid,status:m.status,errorCode:m.errorCode}))));
