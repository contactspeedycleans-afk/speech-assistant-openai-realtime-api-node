...
export function createOpenAiToolHandlers({
    ...
    async function handleBillingLookupTool({
        ...
        try {
            ...
            if (cacheIsFresh && cacheHasBilling && cacheBelongsToCaller) {
                ...
                // A successful live lookup must still be returned to Lisa if
    ...
    async function cancelBookingAction({
        ...
        cancellationScope = 'single_visit',
        customerBookings = []
    }) {
        const suppliedBookingId = bookingId;
        ...
        if (cancellationScope !== 'single_visit') {
            return {
                success: false,
        ...
        try {
            ...
            console.error(
                'Lisa Octopus cancellation failed:',
                error
            );
            ...
            return {
                ...
                customer_message:
                    'I could not complete that cancellation in OctopusPro. The appointment has not been cancelled.'
            };
        }
    }

    async function rescheduleBookingAction({
        bookingId,
        requestedDate,
        requestedStartTime,
        customerConfirmed = false,
        rescheduleScope = 'single_visit',
        customerBookings = []
    }) {
        const suppliedBookingId = bookingId;
        ...
        requestedStartTime =
            String(requestedStartTime || '').trim();
        ...
        if (rescheduleScope !== 'single_visit') {
            return {
                success: false,
                outcome: 'staff_review_required',
                error:
        ...
        try {
            ...
            console.error(
                'Lisa Octopus rescheduling failed:',
                error
            );
    ...
    async function handleCreateBookingTool({
        response,
        openAiWs,
        WebSocket,
        callerPhone = '',
        ...
        if (
            response.type !== 'response.function_call_arguments.done' ||
            response.name !== 'create_octopus_booking'
        ) {
            return false;
        }
        ...
        try {
            ...
            console.error('Could not parse create-booking tool arguments:', error);
        }
        ...
        const customerName = String(pick(
            toolArguments.customerName,
            bookingDefaults.customerName
        )).trim();

        const nameParts = customerName.split(/\s+/).filter(Boolean);
        const customerFirstName = String(pick(
            toolArguments.customerFirstName,
            bookingDefaults.customerFirstName,
            nameParts[0]
        )).trim();
        const customerLastName = String(pick(
            toolArguments.customerLastName,
            bookingDefaults.customerLastName,
            nameParts.slice(1).join(' ')
        )).trim();

        let streetNumber = String(pick(toolArguments.streetNumber, bookingDefaults.streetNumber)).trim();
        let street = String(pick(toolArguments.street, bookingDefaults.street)).trim();
        let city = String(pick(toolArguments.city, bookingDefaults.city)).trim();
        let state = String(pick(toolArguments.state, bookingDefaults.state)).trim();
        let zip = String(pick(toolArguments.zip, bookingDefaults.zip)).trim();
        const unit = String(pick(toolArguments.unit, bookingDefaults.unit)).trim();

        const suppliedServiceAddress = String(pick(
            toolArguments.serviceAddress,
            bookingDefaults.serviceAddress
        )).trim();

        // Angi/Make sometimes supplies only one complete address string. Parse it
        // into the split fields required by the proven Octopus Playwright creator.
        if (suppliedServiceAddress && (!streetNumber || !street || !city || !state || !zip)) {
            const match = suppliedServiceAddress.match(
                /^\s*(\d+[A-Za-z-]*)\s+([^,]+),\s*([^,]+),\s*([A-Za-z]{2})\s+(\d{5}(?:-\d{4})?)\s*$/
            );

            if (match) {
                streetNumber = streetNumber || match[1].trim();
                street = street || match[2].trim();
                city = city || match[3].trim();
                state = state || match[4].trim().toUpperCase();
                zip = zip || match[5].trim();
            }
        }
        ...
        const phase = String(toolArguments.phase || 'finalize').trim().toLowerCase();
        const payload = {
            action: 'create',
            phase,
            draftId: String(toolArguments.draftId || '').trim(),
            customerConfirmed: toolArguments.customerConfirmed === true,
            source: 'EMMA_OUTBOUND_VOICE',
            customerId: String(pick(toolArguments.customerId, bookingDefaults.customerId)).trim(),
            customerName: customerName || [customerFirstName, customerLastName].filter(Boolean).join(' '),
            customerFirstName,
            customerLastName,
            customerPhone: String(pick(toolArguments.customerPhone, callerPhone, bookingDefaults.customerPhone)).trim(),
            customerEmail: String(pick(toolArguments.customerEmail, bookingDefaults.customerEmail)).trim(),
            serviceAddress: String(pick(suppliedServiceAddress, assembledAddress)).trim(),
            streetNumber,
            street,
            city,
            state,
            zip,
            unit,
            serviceType: String(pick(toolArguments.serviceType, bookingDefaults.serviceType)).trim(),
            serviceName: String(pick(toolArguments.serviceName, toolArguments.serviceType, bookingDefaults.serviceType)).trim(),
            requestedDate: String(pick(toolArguments.requestedDate, bookingDefaults.requestedDate)).trim(),
            requestedStartTime: String(pick(toolArguments.requestedStartTime, bookingDefaults.requestedStartTime)).trim(),
            arrivalWindow: String(pick(toolArguments.arrivalWindow, bookingDefaults.arrivalWindow)).trim(),
            durationMinutes: Number(pick(toolArguments.durationMinutes, bookingDefaults.durationMinutes, 120)) || 120,
            quotedPrice: Number(pick(toolArguments.quotedPrice, bookingDefaults.quotedPrice, 0)) || 0,
            price: Number(pick(toolArguments.quotedPrice, bookingDefaults.quotedPrice, 0)) || 0,
            recurringFrequency: String(pick(toolArguments.recurringFrequency, bookingDefaults.recurringFrequency, 'one_time')).trim(),
            frequency: String(pick(toolArguments.recurringFrequency, bookingDefaults.recurringFrequency, 'one_time')).trim(),
            specialNotes: String(pick(toolArguments.specialNotes, bookingDefaults.specialNotes)).trim(),
            accessInstructions: String(pick(toolArguments.accessInstructions, bookingDefaults.accessInstructions)).trim()
        };

        let toolResult;

        if (typeof createBookingAction !== 'function') {
            toolResult = {
                success: false,
                verified_created_in_octopus: false,
                outcome: 'booking_action_unavailable',
                error: 'Live booking action is unavailable.'
            };
        } else if (payload.phase !== 'draft' && payload.customerConfirmed !== true) {
            toolResult = {
                success: false,
                verified_created_in_octopus: false,
                outcome: 'confirmation_required',
            ...
            try {
                console.log('Lisa live create booking request:', {
                ...
                toolResult = await createBookingAction(payload);

                if (payload.phase === 'draft') {
                    const draftId = String(toolResult?.draftId || '');
                    toolResult = {
                        ...(toolResult || {}),
                        success: toolResult?.success === true && Boolean(draftId),
                        draftId,
                        customer_message:
                            'The booking form is loading in the background. Continue the conversation naturally, explain the confirmation form, confirm the final details, then call create_octopus_booking again with phase finalize and this draftId. Do not say the appointment is booked yet.'
                    };
                } else {
                const bookingNumber = toolResult?.bookingNumber || toolResult?.booking_number || '';
                const verified =
                    toolResult?.success === true &&
                    toolResult?.verified_created_in_octopus === true &&
                ...
                if (!verified) {
                    toolResult = {
                        ...
                        verified_created_in_octopus: false,
                        customer_message:
                            'The appointment details were submitted, but the live confirmation number was not verified. Do not tell the customer they are officially booked and do not invent a BOK number.'
                    };
                } else {
                    toolResult = {
                        ...toolResult,
                        bookingNumber,
                        customer_message:
                            `The appointment is officially booked. Confirmation number: ${bookingNumber}. Read this BOK number to the customer twice.`
                    };
                }
                }
            } catch (error) {
                console.error('Lisa live create booking failed:', error);
                toolResult = {
                    success: false,
                    verified_created_in_octopus: false,
                    outcome: 'automation_error',
                    error: error?.message || 'Unable to create the booking.',
                    customer_message:
                        'The live confirmation could not be verified. Do not invent a booking number or claim the appointment is officially booked.'
                };
            }
        }
