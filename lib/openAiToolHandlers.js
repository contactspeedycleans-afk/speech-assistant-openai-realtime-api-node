...
export function createOpenAiToolHandlers({
    ...
    async function handleCreateBookingTool({
        ...
        if (typeof createBookingAction !== 'function') {
        ...
        } else if (payload.customerConfirmed !== true) {
            toolResult = {
                success: false,
                verified_created_in_octopus: false,
                outcome: 'confirmation_required',
            ...
            try {
                ...
                if (!verified) {
                    const stillProcessing = toolResult?.outcome === 'processing';
                    toolResult = {
                        ...(toolResult || {}),
                        success: false,
                        verified_created_in_octopus: false,
                        customer_message:
                            stillProcessing
                                ? (toolResult.customer_message ||
                                    'I have submitted your appointment. Your booking number will be texted and emailed as soon as Octopus finishes processing it. Do not say Octopus confirmed it yet and do not call the booking tool again.')
